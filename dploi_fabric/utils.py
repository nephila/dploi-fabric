...
class Configuration(object):
    ...
    defaults = {
        ...
        "gunicorn": {"workers": 2, "maxrequests": 0, "timeout": None, "bind": None, "version": None,},
        "celery": {
            "enabled": False,
            "concurrency": 1,
        ...
        "static": {},
        "redis": {"enabled": False, "appendonly": "no", "template": app_package_path("templates/redis/redis.conf"),},
        "memcached": {"enabled": True, "size": 64,},
        "processes": {},
        "sendfile": {},
        "environment": {},
        "nginx": {
        ...
        "newrelic": {"enabled": False, "config_file": "newrelic.ini", "environment_name": "", "license": "",},
        "logdir": None,
    }
    ...
    def load_sites(self, config_file_content=None, env_dict=None):
        ...
        if not config_file_content:
            ...
            output = StringIO(config_file_content)
        ...
        for site in config.section_namespaces("django") or ["main"]:
            ...
            if attr_dict.get("checkout").get("tool") == "buildout":
                ...
                attr_dict["django"]["cmd"] = posixpath.join(
                    attr_dict.get("deployment").get("path"), attr_dict.get("django").get("cmd")
                )
            else:
                # e.g. manage.py -> /home/username/app/bin/python /home/username/app/manage.py
                new_django_cmd = [
                    posixpath.join(attr_dict.get("deployment").get("path"), "bin/python",),
                    posixpath.join(
            ...
            if attr_dict["newrelic"]["enabled"]:
                attr_dict["django"]["cmd"] = (
                    posixpath.join(attr_dict.get("deployment").get("path"), "bin/newrelic-admin")
                    + " run-program "
                    + attr_dict["django"]["cmd"]
                )
    ...
    def processes(self, site, env_dict):
        ...
        * type
        ...
        socket = posixpath.normpath(
            posixpath.join(env_dict.get("path"), "..", "tmp", "%s_%s_gunicorn.sock" % (env_dict.get("user"), site))
        )  # Asserts pony project layout
        ...
        cmd = (
            env_dict.get("path")
            if not site_dict.get("newrelic").get("enabled")
            else "%sbin/newrelic-admin run-program %s" % (env_dict.get("path"), env_dict.get("path"))
        )
        cmd += "bin/gunicorn"
        ...
        gunicorn_command = render_template(gunicorn_command_template_path, gunicorn_cmd_context, strip_newlines=True,)
        ...
        if site_dict.get("memcached").get("enabled"):
            memcached_socket = posixpath.normpath(
                posixpath.join(
                    env_dict.get("path"), "..", "tmp", "%s_%s_memcached.sock" % (env_dict.get("user"), site)
                )
            )  # Asserts pony project layout
        ...
        if site_dict.get("celery").get("enabled"):
            ...
            celeryd_command = render_template(
                celeryd_command_template_path, celeryd_command_context, strip_newlines=True,
            )
            ...
            if conf.get("celerycam"):
                ...
                celerycam_command = render_template(
                    celerycam_command_template_path, celerycam_command_context, strip_newlines=True,
                )
        ...
        if site_dict.get("redis").get("enabled"):
            ...
            redis_socket = posixpath.normpath(
                posixpath.join(env_dict.get("path"), "..", "tmp", process_name + ".sock")
            )  # Asserts pony project layout
            process_dict[process_name] = {
                "command": "/usr/bin/redis-server %s"
                % posixpath.normpath(posixpath.join(env_dict.get("path"), "..", "config", process_name + ".conf")),
                "port": None,
                "socket": redis_socket,
                "type": "redis",
                "priority": 20,
    ...
    def deployment(self, site, env_dict):
        ...
        gunicorn_dict["maxrequests"] = env_dict.get("gunicorn", {}).get(
            "maxrequests", gunicorn_dict.get("maxrequests")
        )
        ...
        celery_dict["maxtasksperchild"] = env_dict.get("celery", {}).get(
            "maxtasksperchild", celery_dict.get("maxtasksperchild")
        )
        ...
        nginx_dict["location_settings"] = {
            "client_max_body_size": env_dict.get("nginx", {}).get(
                "client_max_body_size", nginx_dict.get("client_max_body_size")
            ),
        }
        ...
        supervisor_dict["daemon_template"] = env_dict.get("supervisor", {}).get(
            "daemon_template", supervisor_dict.get("daemon_template")
        )
        supervisor_dict["group_template"] = env_dict.get("supervisor", {}).get(
            "group_template", supervisor_dict.get("group_template")
        )
        supervisor_dict["gunicorn_command_template"] = env_dict.get("supervisor", {}).get(
            "gunicorn_command_template", supervisor_dict.get("gunicorn_command_template")
        )
        supervisor_dict["celeryd_command_template"] = env_dict.get("supervisor", {}).get(
            "celeryd_command_template", supervisor_dict.get("celeryd_command_template")
        )
        supervisor_dict["celeryd_command_template"] = env_dict.get("supervisor", {}).get(
            "celeryd_command_template", supervisor_dict.get("celeryd_command_template")
        )
        supervisor_dict["supervisorctl_command"] = env_dict.get("supervisor", {}).get(
            "supervisorctl_command", supervisor_dict.get("supervisorctl_command")
        )
        supervisor_dict["supervisord_command"] = env_dict.get("supervisor", {}).get(
            "supervisord_command", supervisor_dict.get("supervisord_command")
        )
        supervisor_dict["use_global_supervisord"] = env_dict.get("supervisor", {}).get(
            "use_global_supervisord", supervisor_dict.get("use_global_supervisord")
        )
        if supervisor_dict["supervisorctl_command"] is None:
            if supervisor_dict["use_global_supervisord"]:
                ...
                ] = "supervisorctl --config={}../config/supervisord.conf".format(deployment_dict["path"])
        ...
        newrelic_dict["config_file"] = env_dict.get("newrelic", {}).get(
            "config_file", newrelic_dict.get("config_file")
        )
        if not newrelic_dict["config_file"].startswith("/"):
            newrelic_dict["config_file"] = posixpath.abspath(
                posixpath.join(deployment_dict["path"], newrelic_dict["config_file"],)
            )
        self.sites[site]["environment"]["NEW_RELIC_CONFIG_FILE"] = newrelic_dict["config_file"]
        newrelic_dict["environment_name"] = env_dict.get("newrelic", {}).get(
            "environment_name", newrelic_dict.get("environment_name")
        )
        if newrelic_dict["environment_name"]:
            self.sites[site]["environment"]["NEW_RELIC_ENVIRONMENT"] = newrelic_dict["environment_name"]

        newrelic_dict["license_key"] = env_dict.get("newrelic", {}).get(
            "license_key", newrelic_dict.get("license_key")
        )
...
def download_media(to_dir="./tmp/media/", from_dir="../upload/media/"):
    ...
    * Example: download_media:from_dir="py_src/project/media/"
    ...
    local(
        'rsync -avz --no-links --progress --exclude=".svn" -e "ssh" %(user)s@%(host_string)s:"%(path)s/%(from_dir)s"'
        % env
        + " "
        + to_dir
    )
...
def upload_media(from_dir="./tmp/media/", to_dir="../upload/media/"):
    ...
    * Example: upload_media:to_dir="py_src/project/media/"
    ...
    env.to_dir = to_dir
    local(
        'rsync -avz --no-links --progress --exclude=".svn" '
        + from_dir
        + ' -e "ssh" %(user)s@%(host_string)s:"%(path)s/%(to_dir)s"' % env
    )
