...
def restart():
    for site, site_config in config.sites.items():
        run(
            "%s restart %s:*" % (site_config["supervisor"]["supervisorctl_command"], get_group_name(site, site_config))
        )
...
def update_config_file(dryrun=False, update_command=update, load_config=True):
    ...
    if dryrun:
        ...
        safe_put(StringIO(output), path)
        safe_put(StringIO(supervisord_conf_output), supervisord_conf_path)
        if load_config:
            update_command()
